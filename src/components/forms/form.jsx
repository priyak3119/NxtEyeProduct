...
const CompanyForm = ({ company, onSubmit, onClose }) => {
  ...
  // Fetch regions
  useEffect(() => {
    if (!selectedCountry) return setRegions([]);
  ...
  const removeCustomField = (id) => setAdditionalFields(additionalFields.filter(f => f.id !== id));
  const updateCustomFieldValue = (id, value) =>
    setAdditionalFields(additionalFields.map(f => f.id === id ? { ...f, value } : f));
