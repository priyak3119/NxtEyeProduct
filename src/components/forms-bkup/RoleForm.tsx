...
import { Role, Permission, Company, Branch } from '../../types';
...
const RoleForm: React.FC<RoleFormProps> = ({ role, companies, branches, onSubmit, onClose }) => {
  const { register, handleSubmit, formState: { errors }, watch } = useForm<Partial<Role>>({
    defaultValues: role || {}
  });
  ...
  const actions = [
    ...
    { id: 'update', name: 'Edit', color: 'text-yellow-600' },
  ...
  const isPermissionEnabled = (moduleId: string, actionId: string) => {
    const permission = permissions.find(p => p.id === `${moduleId}_${actionId}`);
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-xl p-6">
              <h4 className="text-lg font-semibold text-gray-800 mb-4">Role Information</h4>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      ...
                      {actions.map(action => (
                        <th key={action.id} className="text-center py-3 px-4 font-semibold text-gray-700">
                          {action.name}
                        </th>
                      ))}
                    </tr>
                        ...
                        {actions.map(action => (
                          <td key={action.id} className="py-4 px-4 text-center">
                            <label className="inline-flex items-center cursor-pointer">
                              <input
                                type="checkbox"
                                checked={isPermissionEnabled(module.id, action.id)}
