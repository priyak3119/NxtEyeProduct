...
import { Branch, Company, Region, Document, CustomField } from '../../types';
...
const BranchForm: React.FC<BranchFormProps> = ({ branch, companies, onSubmit, onClose }) => {
  const { register, handleSubmit, formState: { errors }, watch } = useForm<Partial<Branch>>({
    defaultValues: branch || {}
  });

  const [documents, setDocuments] = useState<Document[]>(branch?.documents || []);
  const [additionalFields, setAdditionalFields] = useState<CustomField[]>(branch?.additionalFields || []);
