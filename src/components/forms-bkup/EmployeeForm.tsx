...
import { Employee, Company, Branch, Department, Shift, Document, CustomField } from '../../types';
...
const EmployeeForm: React.FC<EmployeeFormProps> = ({ 
  ...
  const { register, handleSubmit, formState: { errors }, watch } = useForm<Partial<Employee>>({
    defaultValues: employee || {}
  });

  const [documents, setDocuments] = useState<Document[]>(employee?.documents || []);
  const [additionalFields, setAdditionalFields] = useState<CustomField[]>(employee?.additionalFields || []);
  const [newFieldName, setNewFieldName] = useState('');
  const [newFieldType, setNewFieldType] = useState<'text' | 'number' | 'date' | 'select' | 'textarea'>('text');
  const [profileImage, setProfileImage] = useState<string | null>(employee?.profileImage || null);
